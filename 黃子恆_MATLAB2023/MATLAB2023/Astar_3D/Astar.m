...
% Inizializzo OPEN
OPEN=[size(pos,1),0,dist3D(Target,Start),dist3D(Target,Start), P(end)];
while min(size(OPEN)) ~= 0 % current_pos ~= Target
    ...
    for i=1:length(pos)
        ...
        if (dist3D(current_pos,pos(i,:))) < raggio
            FS(j)=i;
            j=j+1;
        end
    end
...
% expanding OPEN 
    for i = 1:length(FS)
        ...
        tentative_g=B(current(1))+dist3D(current_pos,neighbor_pos);% f(n)
        neighbor(1)=FS(i); % node number
        neighbor(2)=B(FS(i)); % g(n) neigbor
        neighbor(3)=dist3D(neighbor_pos,Target); % h(n) neighbor
