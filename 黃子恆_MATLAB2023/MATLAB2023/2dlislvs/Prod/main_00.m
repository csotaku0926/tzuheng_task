...
% 定義地面站座標
gs_s_coords = [40.7128, -74.0060]; % 起始地面站紐約
...
%{
planes = [540 550 560 570 580 590 600 610 620 630]; % 各平面衛星高度 (km)
inclinations = [53 53 53 53 53 53 53 53 53 53]; % 各平面的軌道傾角
total_satellites = [10400 2000 2000]; % 各平面的總衛星數量
num_orbits = [400 100 50]; % 各平面的軌道數量
%}
% 準備總衛星數量
...
DBAn_visited_nodes_list = [];
search_count_DBAn_star_list = [];
search_time_DBAn_star_list = [];
DBAn_star_hops_list = [];
RTT_dban_list = [];
total_distance_dban_list = [];
...
for minute = 0:5:60
    ...
    [satellite_path_Dijkstra, total_satellite_distance_Dijkstra, Dijkstra_time, visited_nodes_dijk, search_count_dijk] = A_star_uv(satellite_coords, start_satellite, end_satellite, total_satellites_sum);
    fprintf('Dijkstra algorithm executed %d searches.\n', search_count_dijk);
    search_count_dijk_list = [search_count_dijk_list, search_count_dijk];

    % 使用A*演算法找到最短衛星路徑
    [satellite_path_A_star, total_satellite_distance_A_star, A_star_time, A_visited_nodes, search_count_a_star] = A_star_algorithm(satellite_coords, start_satellite, end_satellite, total_satellites_sum);
    fprintf('A* algorithm executed %d searches.\n', search_count_a_star);
    search_count_A_star_list = [search_count_A_star_list, search_count_a_star];
    BA_search_count_limit = ceil((search_count_a_star/2)*0.9);
    disp(BA_search_count_limit);
    % 使用雙向A*演算法找到最短衛星路徑
    %[satellite_path_BA_star, total_satellite_distance_BA_star, BA_star_time, BA_visited_nodes, search_count_ba_star] = A_star_bidirectional_algorithm2_7(satellite_coords, start_satellite, end_satellite, total_satellites_sum, BA_search_count_limit);
    [satellite_path_BA_star, total_satellite_distance_BA_star, BA_star_time, BA_visited_nodes, search_count_ba_star] = BA_uv(satellite_coords, start_satellite, end_satellite, total_satellites_sum);
    ...
    % 使用DBS演算法找到最短衛星路徑
    %[satellite_path_DBAn_star, total_satellite_distance_DBAn_star, DBAn_star_time, DBAn_visited_nodes, search_count_dban_star] = A_star_bidirectional_algorithm3(satellite_coords, start_satellite, end_satellite, total_satellites_sum);
    [satellite_path_DBAn_star, total_satellite_distance_DBAn_star, DBAn_star_time, DBAn_visited_nodes, search_count_dban_star] = BA_DBS_uv_3(satellite_coords, start_satellite, end_satellite, total_satellites_sum);
    fprintf('DBS algorithm executed %d searches.\n', search_count_dban_star);
    search_count_DBAn_star_list = [search_count_DBAn_star_list, search_count_dban_star];

    % 比較單雙向兩種方法產生的路徑是否相同，若不同則記錄
    if ~isequal(satellite_path_A_star, satellite_path_DBAn_star)
        different_count = different_count + 1;
    end

    % 使用DBS-1演算法找到最短衛星路徑
    [satellite_path_DBA_star, total_satellite_distance_DBA_star, DBA_star_time, DBA_visited_nodes, search_count_dba_star] = A_star_bidirectional_algorithm_dynamic(satellite_coords, start_satellite, end_satellite, total_satellites_sum);
    fprintf('DBS-1 algorithm executed %d searches.\n', search_count_dba_star);
    search_count_DBA_star_list = [search_count_DBA_star_list, search_count_dba_star];

    % 使用DBS-2演算法找到最短衛星路徑
    [satellite_path_DBA2_star, total_satellite_distance_DBA2_star, DBA2_star_time, DBA2_visited_nodes, search_count_dba2_star] = A_star_bidirectional_algorithm_dynamic2(satellite_coords, start_satellite, end_satellite, total_satellites_sum);
    fprintf('DBS-2 algorithm executed %d searches.\n', search_count_dba_star);
    search_count_DBA2_star_list = [search_count_DBA2_star_list, search_count_dba_star];

    % 使用DBS-3演算法找到最短衛星路徑
    [satellite_path_DBA3_star, total_satellite_distance_DBA3_star, DBA3_star_time, DBA3_visited_nodes, search_count_dba3_star] = A_star_bidirectional_algorithm_dynamic3(satellite_coords, start_satellite, end_satellite, total_satellites_sum);
    fprintf('DBS-3 algorithm executed %d searches.\n', search_count_dba_star);
    search_count_DBA3_star_list = [search_count_DBA3_star_list, search_count_dba_star];

    % 計算路徑總距離，包括地面站到最近衛星的距離
    total_distance_dijk = total_satellite_distance_Dijkstra + min_distance_gs_s + min_distance_gs_d;
    total_distance_dijk_list = [total_distance_dijk_list, total_distance_dijk];
    ...
    total_distance_DBAn_Star = total_satellite_distance_DBAn_star + min_distance_gs_s + min_distance_gs_d;
    total_distance_dban_list = [total_distance_dban_list,total_distance_DBAn_Star];   
    ...
    total_distance_DBA2_Star = total_satellite_distance_DBA2_star + min_distance_gs_s + min_distance_gs_d;
    total_distance_dba2_list = [total_distance_dba2_list,total_distance_DBA2_Star];
    total_distance_DBA3_Star = total_satellite_distance_DBA3_star + min_distance_gs_s + min_distance_gs_d;
    total_distance_dba3_list = [total_distance_dba3_list,total_distance_DBA3_Star];

    % 輸出結果 - Dijkstra
    fprintf('Dijkstra演算法花費時間: %.4f秒\n', Dijkstra_time);
    search_time_dijk_list = [search_time_dijk_list, Dijkstra_time];
    ...
    RTT_dijk = (total_distance_dijk/light_speed)*1000;
    ...
    search_time_DBAn_star_list = [search_time_DBAn_star_list, DBAn_star_time];
    fprintf('DBS經過的衛星節點數（包含起始與終點）: %d\n', length(satellite_path_DBAn_star));
    DBAn_star_hops_list = [DBAn_star_hops_list, length(satellite_path_DBAn_star)];
    fprintf('DBS搜尋過的節點數: %d\n', length(DBAn_visited_nodes));
    DBAn_visited_nodes_list = [DBAn_visited_nodes_list, length(DBAn_visited_nodes)];
    fprintf('DBS總路徑距離: %.2f公里\n', total_distance_DBAn_Star);
    RTT_dban = (total_distance_DBAn_Star/light_speed)*1000;
    RTT_dban_list = [RTT_dban_list, RTT_dban];
    fprintf('DBS OneWay Latency: %.2fms\n', RTT_dban);
    % 輸出結果 - DBA Star
    fprintf('DBS-演算法花費時間: %.4f秒\n', DBA_star_time);
    search_time_DBA_star_list = [search_time_DBA_star_list, DBA_star_time];
    fprintf('DBS-經過的衛星節點數（包含起始與終點）: %d\n', length(satellite_path_DBA_star));
    DBA_star_hops_list = [DBA_star_hops_list, length(satellite_path_DBA_star)];
    fprintf('DBS-搜尋過的節點數: %d\n', length(DBA_visited_nodes));
    DBA_visited_nodes_list = [DBA_visited_nodes_list, length(DBA_visited_nodes)];
    fprintf('DBS-總路徑距離: %.2f公里\n', total_distance_DBA_Star);
    RTT_dba = (total_distance_DBA_Star/light_speed)*1000;
    RTT_dba_list = [RTT_dba_list, RTT_dba];
    fprintf('DBS- OneWay Latency: %.2fms\n', RTT_dba);
    % 輸出結果 - DBA Star 2
    fprintf('DBS-2演算法花費時間: %.4f秒\n', DBA2_star_time);
    search_time_DBA2_star_list = [search_time_DBA2_star_list, DBA2_star_time];
    fprintf('DBS-2經過的衛星節點數（包含起始與終點）: %d\n', length(satellite_path_DBA2_star));
    DBA2_star_hops_list = [DBA2_star_hops_list, length(satellite_path_DBA2_star)];
    fprintf('DBS-2搜尋過的節點數: %d\n', length(DBA2_visited_nodes));
    DBA2_visited_nodes_list = [DBA2_visited_nodes_list, length(DBA2_visited_nodes)];
    fprintf('DBS-2總路徑距離: %.2f公里\n', total_distance_DBA2_Star);
    RTT_dba2 = (total_distance_DBA2_Star/light_speed)*1000+length(satellite_path_DBA2_star);
    RTT_dba2_list = [RTT_dba2_list, RTT_dba2];
    fprintf('DBS-2 OneWay Latency: %.2fms\n', RTT_dba2);
    % 輸出結果 - DBA Star 3
    fprintf('DBS-3演算法花費時間: %.4f秒\n', DBA3_star_time);
    search_time_DBA3_star_list = [search_time_DBA3_star_list, DBA3_star_time];
    fprintf('DBS-3經過的衛星節點數（包含起始與終點）: %d\n', length(satellite_path_DBA3_star));
    DBA3_star_hops_list = [DBA3_star_hops_list, length(satellite_path_DBA3_star)];
    fprintf('DBS-3搜尋過的節點數: %d\n', length(DBA3_visited_nodes));
    DBA3_visited_nodes_list = [DBA3_visited_nodes_list, length(DBA3_visited_nodes)];
    fprintf('DBS-3總路徑距離: %.2f公里\n', total_distance_DBA3_Star);
    RTT_dba3 = (total_distance_DBA3_Star/light_speed)*1000+length(satellite_path_DBA3_star);
    RTT_dba3_list = [RTT_dba3_list, RTT_dba3];
    fprintf('DBS-3 OneWay Latency: %.2fms\n', RTT_dba3);
    ...
    %DBS-每段距離
    fprintf('DBS- Path 0: 地面站 紐約 到 衛星%d, 距離：%.2f公里,', satellite_path_DBA_star(1), min_distance_gs_s);
    for i = 1:length(satellite_path_DBA_star) - 1
        ...
        fprintf('Path %d: 衛星%d 到 衛星%d, 距離：%.2f公里,', i, satellite_path_DBA_star(i), satellite_path_DBA_star(i+1), distance);
    end
...
%{
    ...
    % 繪製BAD路徑上的節點，用紅色表示
    scatter3(satellite_coords(DBA_visited_nodes, 1), satellite_coords(DBA_visited_nodes, 2), satellite_coords(DBA_visited_nodes, 3), 'g');
    % 繪製路徑
...
fprintf('A* vs BA*:%.2f\n', different_count);
%組合所有要展示的數據
RTT_all_data = [RTT_dijk_list', RTT_a_list', RTT_ba_list', RTT_dban_list', RTT_dba_list', RTT_dba2_list', RTT_dba3_list'];
hops_all_data = [Dijkstra_hops_list', BA_star_hops_list', A_star_hops_list', DBAn_star_hops_list', DBA_star_hops_list', DBA2_star_hops_list', DBA3_star_hops_list'];
visited_all_data = [visited_nodes_dijk_list', A_visited_nodes_list', BA_visited_nodes_list', DBAn_visited_nodes_list', DBA_visited_nodes_list', DBA2_visited_nodes_list', DBA3_visited_nodes_list'];
search_count_all_data = [search_count_dijk_list', search_count_A_star_list', search_count_BA_star_list', search_count_DBAn_star_list', search_count_DBA_star_list', search_count_DBA2_star_list', search_count_DBA3_star_list'];
search_time_all_data = [search_time_dijk_list', search_time_A_star_list', search_time_BA_star_list', search_time_DBAn_star_list', search_time_DBA_star_list', search_time_DBA2_star_list', search_time_DBA3_star_list'];
total_distance_all_data = [total_distance_dijk_list', total_distance_a_list', total_distance_ba_list', total_distance_dban_list', total_distance_dba_list', total_distance_dba2_list', total_distance_dba3_list'];
...
%顯示平均值
fprintf('OneWay Latency Average (ms): Dijkstra: %.4f, A*: %.4f, BA*: %.4f, DBS: %.4f, DBS-1: %.4f, DBA2*: %.4f, DBA3*: %.4f\n', RTT_all_data_mean);
fprintf('Hops Average: Dijkstra: %.1f, A*: %.1f, BA*: %.1f, DBS: %.1f, DBS-1: %.1f, DBA2*: %.1f, DBA3*: %.1f\n', hops_all_data_mean);
fprintf('Visited nodes Average: Dijkstra: %.1f, A*: %.1f, BA*: %.1f, DBS: %.1f, DBS-1: %.1f, DBA2*: %.1f, DBA3*: %.1f\n', visited_all_data_mean);
fprintf('Search count Average: Dijkstra: %.1f, A*: %.1f, BA*: %.1f, DBS: %.1f, DBS-1: %.1f, DBA2*: %.1f, DBA3*: %.1f\n', search_count_all_data_mean);
fprintf('Search time Average: Dijkstra: %.2f, A*: %.2f, BA*: %.2f, DBS: %.2f, DBS-1: %.2f, DBA2*: %.2f, DBA3*: %.2f\n', search_time_all_data_mean);
fprintf('Total distance Average (km): Dijkstra: %.4f, A*: %.4f, BA*: %.4f, DBS: %.4f, DBS-1: %.4f, DBA2*: %.4f, DBA3*: %.4f\n', total_distance_all_data_mean);
