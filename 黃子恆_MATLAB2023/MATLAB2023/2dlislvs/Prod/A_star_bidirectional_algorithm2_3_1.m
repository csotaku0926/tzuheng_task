% 雙向A*輔助函數
function [path, total_distance, elapsed_time, visited_nodes, search_count] = A_star_bidirectional_algorithm2_3_1(satellite_coords, start_satellite, end_satellite, total_satellites_sum)
    tic; % 開始計時
    ...
    f_cost_start(start_satellite) = projection_distance(start_satellite, end_satellite, satellite_coords);
    f_cost_end(end_satellite) = projection_distance(end_satellite, start_satellite, satellite_coords);
    ...
    while ~isempty(open_list_start) && ~isempty(open_list_end)
        ...
        % 如果所有未探索節點的f值都大於最佳路徑成本，則結束搜索
        if ~isempty(open_list_start) && ~isempty(open_list_end)
            min_f_cost_start = min(f_cost_start(open_list_start));
            min_f_cost_end = min(f_cost_end(open_list_end));
            if min(min_f_cost_start, min_f_cost_end) > best_path_cost
                path = best_path;
    ...
    total_distance = 0;
    elapsed_time = toc;
    visited_nodes = union(closed_list_start, closed_list_end);
end

% 潛在函數：投影距離
function potential = projection_distance(current_satellite, target_satellite, satellite_coords)
    direction_vector = satellite_coords(target_satellite, 1:3) - satellite_coords(current_satellite, 1:3);
    direction_vector = direction_vector / norm(direction_vector);
    potential_vector = satellite_coords(current_satellite, 1:3) - satellite_coords(target_satellite, 1:3);
    potential = dot(potential_vector, direction_vector);
end
...
function [g_cost, f_cost, open_list, came_from] = expand_node(satellite_coords, neighbors, current_satellite, target_satellite, g_cost, f_cost, open_list, closed_list, came_from)
    for neighbor = neighbors'
        ...
        f_cost(neighbor) = g_cost(neighbor) + projection_distance(neighbor, target_satellite, satellite_coords);
    end
end
