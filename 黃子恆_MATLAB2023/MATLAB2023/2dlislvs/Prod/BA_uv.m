% 雙向 A* 輔助函數
function [path, total_distance, elapsed_time, visited_nodes_dijk, search_count_dijk] = BA_uv(satellite_coords, start_satellite, end_satellite, total_satellites_sum)
    ...
    while ~isempty(unvisited_start) && ~isempty(unvisited_end)
        ...
        % 從目標點開始搜索
        [~, idx_end] = min(distance_end(unvisited_end) + estimate_cost(satellite_coords, unvisited_end, start_satellite));
