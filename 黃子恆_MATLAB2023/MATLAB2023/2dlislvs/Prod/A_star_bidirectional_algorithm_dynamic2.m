% 主函數
function [path, total_distance, elapsed_time, visited_nodes, search_count] = A_star_bidirectional_algorithm_dynamic2(satellite_coords, start_satellite, end_satellite, total_satellites_sum)
    ...
    % 如果路徑長度超過3，才可以從第二個節點開始刪除連續兩個節點
    if length(path) > 3
        % 從第二個節點開始刪除連續兩個節點
        deleted_nodes = path(2:3);
        %disp(deleted_nodes);
        % 重新定義起點和終點
        start_satellite = path(1);
        end_satellite = path(4);
        % 計算新的路徑
        ...
        path = [path(1); new_path; path(4:end)];
