...
function [path, total_distance, elapsed_time, visited_nodes_dijk, search_count_dijk] = BA_DBS_uv_2(satellite_coords, start_satellite, end_satellite, total_satellites_sum)
    ...
    while ~isempty(unvisited_start) && ~isempty(unvisited_end)
        ...
        % 從起始點開始搜索
        [~, idx_start] = min(distance_start(unvisited_start) + estimate_cost(satellite_coords, unvisited_start, end_satellite, distance_weight, angle_weight));
        ...
        % 從目標點開始搜索
        [~, idx_end] = min(distance_end(unvisited_end) + estimate_cost(satellite_coords, unvisited_end, start_satellite, distance_weight, angle_weight));
...
% 估計函數，包含距離與角度的考量
function cost = estimate_cost(satellite_coords, nodes, target, distance_weight, angle_weight)
    ...
    for i = 1:length(nodes)
        ...
        cost(i) = distance_weight * distance_cost - angle_weight * (1 - cos(angle_cost));
    end
end
