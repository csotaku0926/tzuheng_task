...
for minute = 0:5:5
    ...
    % 使用DBA*演算法找到最短衛星路徑
    [satellite_path_BAD_star, total_satellite_distance_BAD_star, BAD_star_time, BAD_visited_nodes, search_count_bad_star] = A_star_bidirectional_algorithm_dynamic(satellite_coords, start_satellite, end_satellite, total_satellites_sum);
    fprintf('BAD* algorithm executed %d searches.\n', search_count_bad_star);
    % 計算路徑總距離，包括地面站到最近衛星的距離
    total_distance_BA_Star = total_satellite_distance_BA_star + min_distance_gs_s + min_distance_gs_d;
    total_distance_BAD_Star = total_satellite_distance_BAD_star + min_distance_gs_s + min_distance_gs_d;
    % 輸出結果BA*
    ...
    % 輸出結果BA*D
    fprintf('BAD*演算法花費時間: %.4f秒\n', BAD_star_time);
    fprintf('BAD*經過的衛星節點數（包含起始與終點）: %d\n', length(satellite_path_BAD_star));
    %BAD_star_hops_list = [BAD_star_hops_list, length(satellite_path_BAD_star)];
    fprintf('BAD*搜尋過的節點數: %d\n', length(BAD_visited_nodes));
    %BAD_visited_nodes_list = [BAD_visited_nodes_list, length(BAD_visited_nodes)];
    fprintf('BAD*總路徑距離: %.2f公里\n', total_distance_BAD_Star);
    RTT_bad = (total_distance_BAD_Star/light_speed)*1000+length(satellite_path_BAD_star);
    %RTT_bad_list = [RTT_bad_list, RTT_bad];
    fprintf('BAD* OneWay Latency: %.2fms\n', RTT_bad);   
    ...
    %BA*D每段距離
    fprintf('BAD Path 0: 地面站 紐約 到 衛星%d, 距離：%.2f公里,', satellite_path_BAD_star(1), min_distance_gs_s);
    for i = 1:length(satellite_path_BAD_star) - 1
        distance = distance_between(satellite_coords, satellite_path_BAD_star(i), satellite_path_BAD_star(i+1));
        fprintf('Path %d: 衛星%d 到 衛星%d, 距離：%.2f公里,', i, satellite_path_BAD_star(i), satellite_path_BAD_star(i+1), distance);
    end
    fprintf('Path %d: 衛星%d 到 地面站 雪梨, 距離：%.2f公里\n', length(satellite_path_BAD_star), satellite_path_BAD_star(end), min_distance_gs_d);
    ...
    % 繪製BAD路徑上的節點，用紅色表示
    scatter3(satellite_coords(BAD_visited_nodes, 1), satellite_coords(BAD_visited_nodes, 2), satellite_coords(BAD_visited_nodes, 3), 'g');
    % 繪製路徑
    path_coords = [gs_s_3D; satellite_coords(satellite_path_BAD_star, 1:3); gs_d_3D];
    plot3(path_coords(:, 1), path_coords(:, 2), path_coords(:, 3), 'r', 'LineWidth', 2);
    for i = 1:length(satellite_path_BAD_star) - 1
        distance = distance_between(satellite_coords, satellite_path_BAD_star(i), satellite_path_BAD_star(i+1));
        mid_point = (satellite_coords(satellite_path_BAD_star(i), :) + satellite_coords(satellite_path_BAD_star(i+1), :)) / 2;
