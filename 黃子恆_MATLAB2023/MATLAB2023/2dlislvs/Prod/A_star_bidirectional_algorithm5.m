% 雙向A*輔助函數
function [path, total_distance, elapsed_time, visited_nodes, search_count] = A_star_bidirectional_algorithm5(satellite_coords, start_satellite, end_satellite, total_satellites_sum)
    tic; % 開始計時
    ...
    while ~isempty(open_list_start) && ~isempty(open_list_end)
        ...
        if ~isempty(intersect_nodes)
            % 檢查相交節點是否包含索引122
            if any(intersect_nodes == 122)
                fprintf('找到相交節點中包含索引122的節點\n');
            end
            
            % 檢查是否有交集節點的第四個維度包含550
            intersect_nodes_h_value = arrayfun(@(node) satellite_coords(node, 4), intersect_nodes);
            if any(intersect_nodes_h_value == 550)
                % 計算所有交集點的總路徑成本
