% 雙向A*輔助函數
function [path, total_distance, elapsed_time, visited_nodes, search_count] = A_star_bidirectional_algorithm2_2(satellite_coords, start_satellite, end_satellite, total_satellites_sum)
    tic; % 開始計時
    ...
    while ~isempty(open_list_start) && ~isempty(open_list_end)
        ...
        % 擴展從起始點方向的節點
        [g_cost_start, f_cost_start, open_list_start, came_from_start] = expand_node(satellite_coords, neighbors_start, current_satellite_start, current_satellite_end, g_cost_start, f_cost_start, open_list_start, closed_list_start, came_from_start);
        
        % 擴展從終點方向的節點
        [g_cost_end, f_cost_end, open_list_end, came_from_end] = expand_node(satellite_coords, neighbors_end, current_satellite_end, current_satellite_start, g_cost_end, f_cost_end, open_list_end, closed_list_end, came_from_end);
    end
