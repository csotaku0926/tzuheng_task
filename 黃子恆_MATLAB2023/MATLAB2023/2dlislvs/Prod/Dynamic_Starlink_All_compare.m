...
for minute = 0:1:1
    ...
    % 使用反向A*演算法找到最短衛星路徑
    [satellite_path_Ab_star, total_satellite_distance_Ab_star, Ab_star_time, Ab_visited_nodes, search_count_ab_star] = A_star_algorithm(satellite_coords, end_satellite, start_satellite, total_satellites_sum);
    fprintf('Ab* algorithm executed %d searches.\n', search_count_a_star);
    search_count_A_star_list = [search_count_A_star_list, search_count_a_star];
    % 使用雙向A*演算法找到最短衛星路徑
    [satellite_path_BA_star, total_satellite_distance_BA_star, BA_star_time, BA_visited_nodes, search_count_ba_star] = A_star_bidirectional_algorithm3(satellite_coords, start_satellite, end_satellite, total_satellites_sum);
    ...
    fprintf('DBA* algorithm executed %d searches.\n', search_count_dba_star);
...
%{
    ...
    fprintf('DBA*2 algorithm executed %d searches.\n', search_count_dba_star);
    ...
    fprintf('DBA*3 algorithm executed %d searches.\n', search_count_dba_star);
...
%}
    ...
    total_distance_Ab_Star = total_satellite_distance_Ab_star + min_distance_gs_s + min_distance_gs_d;
...
%}
    ...
    % 輸出結果 - Ab Star
    fprintf('Ab*演算法花費時間: %.4f秒\n', A_star_time);
%    search_time_Ab_star_list = [search_time_Ab_star_list, A_star_time];
    fprintf('Ab*經過的衛星節點數（包含起始與終點）: %d\n', length(satellite_path_Ab_star));
%    Ab_star_hops_list = [A_star_hops_list, length(satellite_path_Ab_star)];
    fprintf('Ab*搜尋過的節點數: %d\n', length(A_visited_nodes));
%    Ab_visited_nodes_list = [Ab_visited_nodes_list, length(Ab_visited_nodes)];
    fprintf('Ab*總路徑距離: %.2f公里\n', total_distance_Ab_Star);
    RTT_ab = (total_distance_Ab_Star/light_speed)*1000;
%    RTT_ab_list = [RTT_ab_list, RTT_ab];
    fprintf('Ab* OneWay Latency: %.2fms\n', RTT_a);
    % 輸出結果 - BA Star
...
%}
    ...
    %Ab*每段距離
    fprintf('Ab* Path 0: 地面站 雪梨 到 衛星%d, 距離：%.2f公里,', satellite_path_Ab_star(1), min_distance_gs_d);
    for i = 1:length(satellite_path_Ab_star) - 1
        distance = distance_between(satellite_coords, satellite_path_Ab_star(i), satellite_path_Ab_star(i+1));
        fprintf('Path %d: 衛星%d 到 衛星%d, 距離：%.2f公里,', i, satellite_path_Ab_star(i), satellite_path_Ab_star(i+1), distance);
    end
    fprintf('Path %d: 衛星%d 到 地面站 紐約, 距離：%.2f公里\n', length(satellite_path_Ab_star), satellite_path_Ab_star(end), min_distance_gs_s);
    ...
    %DBA*每段距離
    fprintf('DBA* Path 0: 地面站 紐約 到 衛星%d, 距離：%.2f公里,', satellite_path_DBA_star(1), min_distance_gs_s);
...
%{
    %DBA*2每段距離
    fprintf('DBA*2 Path 0: 地面站 紐約 到 衛星%d, 距離：%.2f公里,', satellite_path_DBA2_star(1), min_distance_gs_s);
    for i = 1:length(satellite_path_DBA2_star) - 1
        distance = distance_between(satellite_coords, satellite_path_DBA2_star(i), satellite_path_DBA2_star(i+1));
        fprintf('Path %d: 衛星%d 到 衛星%d, 距離：%.2f公里,', i, satellite_path_DBA2_star(i), satellite_path_DBA2_star(i+1), distance);
    end
    fprintf('Path %d: 衛星%d 到 地面站 雪梨, 距離：%.2f公里\n', length(satellite_path_DBA2_star), satellite_path_DBA2_star(end), min_distance_gs_d);

    %DBA*3每段距離
    fprintf('DBA*3 Path 0: 地面站 紐約 到 衛星%d, 距離：%.2f公里,', satellite_path_DBA3_star(1), min_distance_gs_s);
    for i = 1:length(satellite_path_DBA3_star) - 1
        distance = distance_between(satellite_coords, satellite_path_DBA3_star(i), satellite_path_DBA3_star(i+1));
        fprintf('Path %d: 衛星%d 到 衛星%d, 距離：%.2f公里,', i, satellite_path_DBA3_star(i), satellite_path_DBA3_star(i+1), distance);
    end
    fprintf('Path %d: 衛星%d 到 地面站 雪梨, 距離：%.2f公里\n', length(satellite_path_DBA3_star), satellite_path_DBA3_star(end), min_distance_gs_d);
%}
%{
    % 繪製3D座標圖 - Dijkstra 路徑
...
%顯示平均值
fprintf('OneWay Latency Average (ms): Dijkstra: %.2f, A*: %.2f, BA*: %.2f, DBA*: %.2f\n', RTT_all_data_mean);
fprintf('Hops Average: Dijkstra: %.2f, A*: %.2f, BA*: %.2f, DBA*: %.2f\n', hops_all_data_mean);
fprintf('Visited nodes Average: Dijkstra: %.2f, A*: %.2f, BA*: %.2f, DBA*: %.2f\n', visited_all_data_mean);
fprintf('Search count Average: Dijkstra: %.2f, A*: %.2f, BA*: %.2f, DBA*: %.2f\n', search_count_all_data_mean);
fprintf('Search time Average: Dijkstra: %.2f, A*: %.2f, BA*: %.2f, DBA*: %.2f\n', search_time_all_data_mean);
fprintf('Total distance Average (km): Dijkstra: %.2f, A*: %.2f, BA*: %.2f, DBA*: %.2f\n', total_distance_all_data_mean);


%箱型圖 - Latency
figure
boxplot(RTT_all_data, 'Labels', {'Dijkstra', 'A*', 'DBA*', 'DBA*(Reconstruct)'})
title('OneWay Latency Comparison (ms)')

figure
boxplot(hops_all_data, 'Labels', {'Dijkstra', 'A*', 'DBA*', 'DBA*(Reconstruct)'})
title('Hops Comparison')

figure
boxplot(visited_all_data, 'Labels', {'Dijkstra', 'A*', 'DBA*', 'DBA*(Reconstruct)'})
title('Visited nodes Comparison')

figure
boxplot(search_count_all_data, 'Labels', {'Dijkstra', 'A*', 'DBA*', 'DBA*(Reconstruct)'})
title('Search count Comparison')

figure
boxplot(total_distance_all_data, 'Labels', {'Dijkstra', 'A*', 'DBA*', 'DBA*(Reconstruct)'})
