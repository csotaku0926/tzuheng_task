% 雙向 A* 輔助函數
function [path, total_distance, elapsed_time, visited_nodes_dijk, search_count_dijk] = BA_DBS_uv_3_2(satellite_coords, start_satellite, end_satellite, total_satellites_sum)
    ...
    while ~isempty(unvisited_start) && ~isempty(unvisited_end)
        ...
        % 從起始點開始搜索
        [~, idx_start] = min(distance_start(unvisited_start) + estimate_cost(satellite_coords, unvisited_start, end_satellite, angle_weight,start_satellite));
        ...
        % 從目標點開始搜索
        [~, idx_end] = min(distance_end(unvisited_end) + estimate_cost(satellite_coords, unvisited_end, start_satellite, angle_weight,end_satellite));
...
% 估計函數，包含角度的考量
function cost = estimate_cost(satellite_coords, nodes, target, angle_weight,source)
    ...
    for i = 1:length(nodes)
        ...
        angle_cost = angle_between( node_coords - source, node_coords);
