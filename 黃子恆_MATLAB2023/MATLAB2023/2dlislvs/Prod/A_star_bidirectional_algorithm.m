% 雙向A*輔助函數
function [path, total_distance, elapsed_time, visited_nodes, search_count] = A_star_bidirectional_algorithm(satellite_coords, start_satellite, end_satellite, total_satellites_sum)
    tic; % 開始計時
    ...
    while ~isempty(open_list_start) && ~isempty(open_list_end)
        % 在while迴圈開始處增加檢查
        if ~isempty(best_solution) && min([f_cost_start(open_list_start); f_cost_end(open_list_end)]) > best_solution_cost
            break;
        end
        ...
        if ~isempty(intersect_nodes)
            ...
            if total_costs(min_cost_index) < best_solution_cost
                best_solution = intersect_node;
                best_solution_cost = total_costs(min_cost_index);
            end
        end
    ...
    if ~isempty(best_solution)
        path_from_start = reconstruct_path(came_from_start, best_solution);
        path_from_end = flip(reconstruct_path(came_from_end, best_solution));
        % 將兩部分路徑合併，並確保交集節點不重複
        path = [path_from_start; path_from_end(2:end)];
        total_distance = best_solution_cost;
