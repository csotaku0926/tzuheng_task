% 
function [path, total_distance, elapsed_time, visited_nodes_astar, search_count_astar] = A_star_bidirectional_algorithm6(satellite_coords, start_satellite, end_satellite, total_satellites_sum)
    ...
    while ~isempty(unvisited_start) && ~isempty(unvisited_end)
        ...
        % 從起始點開始搜索
        [~, idx_start] = min(distance_start(unvisited_start) + heuristic_cost(satellite_coords, unvisited_start, end_satellite));
        ...
        % 從目標點開始搜索
        [~, idx_end] = min(distance_end(unvisited_end) + heuristic_cost(satellite_coords, unvisited_end, start_satellite));
        ...
        % 檢查是否已經可以停止搜索
        if distance_start(current_satellite_start) + heuristic_cost(satellite_coords, current_satellite_start, end_satellite) + distance_end(current_satellite_end) + heuristic_cost(satellite_coords, current_satellite_end, start_satellite) >= mu
