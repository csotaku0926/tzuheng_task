% 主函數
function [path, total_distance, elapsed_time, visited_nodes, search_count] = A_star_bidirectional_algorithm_dynamic2_rdm(satellite_coords, start_satellite, end_satellite, total_satellites_sum)
    ...
    % 如果路徑長度超過3，才可以刪除連續兩個節點
    if length(path) > 3
        % 隨機選取一個節點刪除（除了起點、終點及倒數第二個節點）
        rand_idx = randi([2, length(path)-2]);
        deleted_nodes = path(rand_idx:rand_idx+1);
        disp(deleted_nodes);
        % 重新定義起點和終點
        start_satellite = path(rand_idx-1);
        end_satellite = path(rand_idx+2);
        % 計算新的路徑
        ...
        path = [path(1:rand_idx-1); new_path; path(rand_idx+3:end)];
